...
def pisvm(p, gamma, C, split):
    ...
    test_command = './svm-predict -o -P ' + str(p) + ' ' + val_file + ' ' + model_basename + '_G'+str(gamma)+'_C'+str(C)+' ' + output_name
...
for t in thres_values:
    t = round(t,3)
    f1_binary, _ = pisvm(t, gamma, C, split)
