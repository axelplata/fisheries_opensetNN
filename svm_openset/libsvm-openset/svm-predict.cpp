...
void predict(FILE *input, FILE *output){
	...
	while(readline(input) != NULL){
		...
		if (predict_probability && (svm_type==C_SVC || svm_type==NU_SVC)){
			...
			if(known_class){
				fprintf(output,"first if %g,%g,%g\n",target_label,predict_label, max_prob); //Diduch
				...
				if(max_prob < model->param.openset_min_probability){
                    fprintf(output,"second if %d,%d,%g\n",99,99,max_prob);//Diduch
                    trueneg++;
				}
				else{
                    fprintf(output,"third %d,%g,%g\n",99,predict_label,max_prob);//Diduch
                    falsepos++;
				}
			}
                   //fprintf(output,"%g: %g\n",predict_label,max_prob); Diduch commented this
	    ...
	    fprintf(output,"%g,%g,%g\n",target_label,predict_label,max_prob);
            //fprintf(output,"%g",predict_label); Diduch commented this
            //cleanup scores and votes
            for(int v=0; v<model->nr_class; v++)
                if(scores[v] != NULL)
                    free(scores[v]);
	...
	if (svm_type==NU_SVR || svm_type==EPSILON_SVR ){
        ...
        //open-set
		if(svm_type==ONE_VS_REST_WSVM || svm_type == ONE_WSVM || svm_type == PI_SVM){
