...
class FDWEDataset(Dataset):
    ...
    def _get_data(self):
        ...
        for cl_folder in sorted(os.listdir(f'{self.data_dir}/fdwe/{self.split}')):
            label = int(cl_folder)
            for image_name in sorted(os.listdir(f'{self.data_dir}/fdwe/{self.split}/{cl_folder}')):
                image_path = f'{self.data_dir}/fdwe/{self.split}/{cl_folder}/{image_name}'
...
def get_dataset(dataset, split='train', select_classes=None, target_transform=None):
    if dataset == 'cifar10':
        ...
        transform_train = transforms.Compose([
                                transforms.Resize((224,224)), #original used is 64x64
                                transforms.RandomRotation(20),
                                transforms.RandomHorizontalFlip(0.5),
                                transforms.ToTensor(),
                                lambda x: x if x.shape[0] == 3 else x.repeat(3, 1, 1),
                                transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225]), #imagenet values/ default
                                #transforms.Normalize([0.4275, 0.3894, 0.3853], [0.2209, 0.2276, 0.2344]), #FDWE mean and std
        ...
        train_set = FDWEDataset(data_dir=data_dir,split=split,transform=transform_train)
        test_set = FDWEDataset(data_dir=data_dir,split=split,transform=transform_test)
