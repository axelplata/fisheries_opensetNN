...
#However; there are few instances, so
...
def find_bestThreshold_balAcc(known_preds, known_labels, unknown_preds, unknown_labels, dist_prototypes_known, dist_prototypes_unknown):
    ...
    #Define a threshold per prototype per specie
    threshold_by_prototype = np.zeros((n_species, n_prototypes))
    
    for specie in range(n_species):

        #Considering only the correct classified instances from Known samples to define the threshold
        #Should we consider all the known samples using the ground truth labels?
        ...
        distances_correct_preds = distances_correct_preds [:, specie] #only distances to all prototypes of a specific class (specie)
        ...
        #prototypes used to define the known instances
        unique_prototypes_used = np.unique(prototypes_used)

        #Prototypes not considered to define known classes
            #Prototypes not considered to define unknowns 
        ...
        for prototype in range(n_prototypes):
            ...
            scores_unknown = cp_dist_prototypes_unknown[:,specie,prototype]

            #we used all unknowns instances to define the threshold if there were not unknown instances predicted as the current species or unknown instances linked to the current prototype
            if (prototype not in unique_prototypes_used) or len(scores_unknown) == 0 :
                distances_prototype = np.copy(min_distances)
                std_dist = np.std(distances_prototype)

                scores_unknown = dist_prototypes_unknown[:,specie,prototype] #dist from the current prototype to all unknown instances
                cp_unknown_preds = np.copy(unknown_preds)
                cp_unknown_labels = np.copy(unknown_labels)

            else:
                cp_unknown_preds = np.copy(unknown_preds)
                cp_unknown_labels = np.copy(unknown_labels)
                cp_unknown_labels = cp_unknown_labels[cp_unknown_preds == specie]
                cp_unknown_preds = cp_unknown_preds[cp_unknown_preds == specie]
            ...
            for j, dist in enumerate(distances_prototype):
                ...
                acc_unknown = acc_byThreshold(cp_unknown_preds, cp_unknown_labels, scores_unknown, dist)
