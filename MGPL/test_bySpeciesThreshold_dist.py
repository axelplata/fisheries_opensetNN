...
def test_openset(model, inner_loader, open_loader, threshold_by_specie):
    ...
    _, inner_label, dist_inner_score, kl_inner_score, dist_inner_pred, kl_inner_pred = get_output(model, inner_loader)
    _, open_label, dist_open_score, kl_open_score, dist_open_pred, kl_open_pred = get_output(model, open_loader)
    
    #accuracy considering only known samples
    acc_known = round(accuracy(dist_inner_pred, inner_label)*100,3)

 
    #concatenate all samples info
    full_kl_minDist = np.concatenate((dist_inner_score, dist_open_score)) #kl distance to the closest prototype
    full_labels = np.concatenate((inner_label, open_label)) 
    full_preds = np.concatenate((dist_inner_pred, dist_open_pred))
    ...
    print('Known samples accuracy:', acc_byThreshold(dist_inner_score, dist_inner_pred, inner_label, threshold_by_specie))
    print('Unknown samples accuracy:', acc_byThreshold(dist_open_score, dist_open_pred, open_label, threshold_by_specie))
...
if __name__ == '__main__':
    ...
    threshold_by_specie = {0:3.2605362, 1:4.310753, 2:3.2469597, 3:4.5548153}
