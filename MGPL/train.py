...
if __name__ == '__main__':
    ...
    print(known_dataset, 'known', known_classes,'unknown val', unknown_val, 'unknown test', unknown_test)

    train_set = get_dataset(known_dataset, 'train', known_classes, 'reindex')
    val_set = get_dataset(known_dataset, 'val', known_classes, 'reindex')
    open_set = get_dataset(unknown_dataset, 'val', unknown_val, 'open')
    
    print('train set', train_set)
    print('this is the val set', val_set)

    train_loader = DataLoader(train_set, batch_size=args.batch_size, shuffle=True, num_workers=4)
    val_loader = DataLoader(val_set, batch_size=args.batch_size, shuffle=False, num_workers=4)#modified diduch
    open_loader = DataLoader(open_set, batch_size=args.batch_size, shuffle=False, num_workers=4)#modified diduch
   

    print('train', len(train_loader.dataset))
    print('val', len(val_loader.dataset))
    print('open set', len(open_loader.dataset))
    ...
    for epoch in range(args.epoch):
        ...
        if epoch in [50, 80]: 
            mgplnet.prototypes = reset_prototypes(mgplnet, train_loader, n_sub_prototypes=args.k)
